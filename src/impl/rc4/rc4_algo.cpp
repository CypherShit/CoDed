...
RC4Algo::RC4Algo() { 
    InitializeKeyBlock(GenerateRandomKey());
}

RC4Algo::RC4Algo(std::string&& key) {
    InitializeKeyBlock(std::move(key));
}

RC4Algo::RC4Algo(const std::string& key) {
    InitializeKeyBlock(key);
}
...
std::string RC4Algo::Process(std::string_view payload) const {
    ...
    for (size_t k = 0; k < payload.size(); ++k) {
        ...
        std::uint8_t rnd = key_block[(key_block[i] + key_block[j]) % KEY_BLOCK_SIZE];
